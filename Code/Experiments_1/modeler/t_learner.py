...
class T_Learner(nn.Module):
    ...
    def create_model(self):
        ...
        for i in range(self.layer_number-1):
            self.net0.add_module(f'fc{i+1}', layers.FullyConnected(self.hidden_size, self.hidden_size, self.dropout, [nn.ReLU()]))
            self.net1.add_module(f'fc{i+1}', layers.FullyConnected(self.hidden_size, self.hidden_size, self.dropout, [nn.ReLU()]))
        self.net0.add_module(f'fc_{self.layer_number}', layers.FullyConnected(self.hidden_size, self.output_size))
        self.net1.add_module(f'fc_{self.layer_number}', layers.FullyConnected(self.hidden_size, self.output_size))
